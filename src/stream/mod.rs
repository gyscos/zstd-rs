...
macro_rules! readwritecommon {
    ($readwrite:ident) => (
            ...
            pub fn include_dictid(&mut self, include_dictid: bool) -> io::Result<()> {
                self.$readwrite
                    .operation_mut()
                    .set_parameter(zstd_safe::CParameter::DictIdFlag(include_dictid))
            }

            /// Enables or disabled storing of the contentsize
            pub fn include_contentsize(&mut self, include_contentsize: bool) -> io::Result<()> {
                self.$readwrite
                    .operation_mut()
                    .set_parameter(zstd_safe::CParameter::ContentSizeFlag(include_contentsize))
            }

            /// Enables or disable the magic bytes at the beginning of each frame. If disabled, include_magicbytes must also be called on the decoder
            pub fn include_magicbytes(&mut self, include_magicbytes: bool) -> io::Result<()> {
                self.$readwrite
                    ...
                    .set_parameter(if include_magicbytes {
                        ...
                        zstd_safe::CParameter::Format(zstd_safe::FrameFormat::Magicless)
                    })
            }
    )
}